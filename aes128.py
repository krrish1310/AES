...
def derive_key_from_password(password):
    return list(hashlib.sha256(password.encode()).digest()[:16])
...
def key_expansion(key):
    ...
    for i in range(4, 44):
        ...
        if i % 4 == 0:
            ...
            temp = [Sbox[b >> 4][b & 0x0F] for b in temp]  # SubWord
            temp[0] ^= Rcon[i // 4]
...
def add_round_key(state, round_key):
    return [[s ^ rk for s, rk in zip(row, rkey)] for row, rkey in zip(state, round_key)]
...
def mix_columns(state):
    return state  # Still a placeholder, implement if needed
...
def decrypt_block(block, key_schedule):
    ...
    for rnd in range(9, 0, -1):
        ...
        state = [[next(i for i in range(256) if Sbox[i >> 4][i & 0x0F] == b) for b in row] for row in state]
        state = add_round_key(state, key_schedule[rnd])
        state = mix_columns(state)  # still dummy, replace with InvMixColumns when implemented
    state = [
        state[0],
        state[1][-1:] + state[1][:-1],
        state[2][-2:] + state[2][:-2],
